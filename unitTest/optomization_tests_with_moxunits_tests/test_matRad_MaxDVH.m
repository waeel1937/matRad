...
function test_suite=test_matRad_MaxDVH
...
import DoseObjectives.matRad_MaxDVH

%% Test 1: Quadratische Abweichung
dos.name = 'Max DVH';
dos.parameterNames = {'d', 'V^{max}'};
dos.parameterTypes = {'dose','numeric'};

scal  = {30,95};

r  =  matRad_MaxDVH(dos,scal);


assertEqual(r.name, 'Max DVH');
assertEqual(r.parameterNames, dos.parameterNames);
assertEqual(r.parameters{1}, 30);
...
% 
% SquaredDev = matRad_MaxDVH.instance();
